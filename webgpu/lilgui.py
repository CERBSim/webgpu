...
class LilGUI:
    ...
    def slider(self, value: float, func: Callable[[RenderObject, float], None],
               objects: list[RenderObject] | RenderObject = [],
               min=0., max=1., label="Slider"):
        self._connect(func, option={ "type" : "slider",
                                     "min" : min,
                                     "max" : max,
                                     "value" : value,
                                     "label" : label },
    ...
    def _connect(self, func, option,
                 render_objects: list[RenderObject] | RenderObject = []):
        ...
        run_code_in_pyodide(f"import webgpu.lilgui; webgpu.lilgui._receive('{self.canvas_id}', '{self.scene_id}', {option}, {render_objects}, '{_encode_data(_encode_function(func))}')")
...
def _receive(canvas_id, scene_id, option, render_objects: list[str], func: str):
    ...
    if option["type"] == "slider":
        ...
        gui.add(slider, option["label"], option["min"], option["max"]).onChange(pyodide.ffi.create_proxy(_func))
